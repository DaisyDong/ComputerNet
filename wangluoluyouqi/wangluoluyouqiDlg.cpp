...
{
// 错误，显示错误信息
sprintf(strbuf, " 接 口 未 能 打 开 。 WinPcap 不 支 持 %s 。 ",
IfInfo[i].DeviceName);
MessageBox(strbuf);
PostMessage(WM_QUIT, 0, 0);
}
...
LOGGER.InsertString(-1,("MAC 地址： "+ MACntoa(IfInfo[i].MACAddr)));
for (j = 0; j < IfInfo[i].ip.GetSize(); j++)
{
LOGGER.InsertString(-1,("IP 地址："+IPntoa(IfInfo[i].ip[j].IPAddr)));
}
}
// 初始化路由表并显示
RouteTable_t rt;
...
{
AfxMessageBox("pcap_next_ex 函数出错!");
}
}
return 0;
}
...
// 日志输出信息
pDlg->LOGGER.InsertString(-1, " 将该对应关系存入 IP－MAC地址映射表中");
}
...
// 日志输出信息
pDlg->LOGGER.InsertString(-1, " 转发缓存区中目的地址是该 MAC 地址的 IP 数据包");
pDlg->LOGGER.InsertString(-1, (" 发送 IP 数据包："+IPntoa(IPf->IPHeader.SrcIP) + "->"+  IPntoa(IPf->IPHeader.DstIP)  +  "  "  +MACntoa(IPf->FrameHeader.SrcMAC )+"->"+MACntoa(IPf->FrameHeader.DesMAC)));
flag = true;
break;
}
}
...
SendPacket_t sPacket;
IPf = (IPFrame_t *) pkt_data;
pDlg->LOGGER.InsertString(-1,  (" 收 到 IP 数 据 包 :"  +
...
{
// 日志输出信息
pDlg->LOGGER.InsertString(-1, " IP 数据包包头校验和错误，丢弃数据包");
return;
}
// 路由查询
...
{
// 错误处理
AfxMessageBox("发送 IP 数据包时出错!");
return;
}
// 日志输出信息
pDlg->LOGGER.InsertString(-1," 转发 IP 数据包：");
...
mMutex.Unlock();
// 日志输出信息
pDlg->LOGGER.InsertString(-1, " 缺少目的 MAC 地址， 将 IP数据包存入转发缓冲区");
pDlg->LOGGER.InsertString(-1, (" 存入转发缓冲区的数据包为："+IPntoa(IPf->IPHeader.SrcIP)+ "->" + IPntoa(IPf->IPHeader.DstIP) + " " +MACntoa(IPf->FrameHeader.SrcMAC)+ "->xx:xx:xx:xx:xx:xx"));
pDlg->LOGGER.InsertString(-1, " 发送 ARP 请求");
// 发送 ARP 请求
...
{
// 日志输出信息
pDlg->LOGGER.InsertString(-1, " 转发缓冲区溢出，丢弃 IP数据包");
pDlg->LOGGER.InsertString(-1, (" 丢弃的 IP 数据包为：" +IPntoa(IPf->IPHeader.SrcIP) + "->"+  IPntoa(IPf->IPHeader.DstIP)  +  "  "  +MACntoa(IPf->FrameHeader.SrcMAC)+ "->xx:xx:xx:xx:xx:xx"));
}
} 
}
}
...
if (type == 11)
{
pDlg->LOGGER.InsertString(-1, " 发送 ICMP 超时数据包：");
}
if (type == 3)
{
pDlg->LOGGER.InsertString(-1, " 发送 ICMP 目的不可达数据包：");
}
...
IPf = (IPFrame_t *)sPacket.PktData;
// 日志输出信息
LOGGER.InsertString(-1, "IP 数据报在转发队列中等待 10 秒后还未能被转发");
LOGGER.InsertString(-1, (" 定时器中删除该 IP 数据报：" +IPntoa(IPf->IPHeader.SrcIP) + "->"+  IPntoa(IPf->IPHeader.DstIP)  +  "  "  +MACntoa(IPf->FrameHeader.SrcMAC)+ "->xx:xx:xx:xx:xx:xx"));
